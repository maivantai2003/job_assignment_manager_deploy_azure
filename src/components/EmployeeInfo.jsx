      ...
      <Popover className='relative'>
        ...
        <>
          <Popover.Button className='group inline-flex items-center outline-none'>
            <span>{getInitials(employee?.nhanVien?.tenNhanVien)}</span>
          </Popover.Button>
              ...
              <div className='flex items-center gap-4 rounded-lg shadow-lg bg-white p-8'>
                <div className='w-16 h-16 bg-blue-600 rounded-full text-white flex items-center justify-center text-2xl '>
                  <span className='text-center font-bold'>
                    {getInitials(employee?.nhanVien?.tenNhanVien)}
                  </span>
                </div>
                <div className='flex flex-col gap-y-1'>
                  <p className='text-black text-xl font-bold'>{employee?.nhanVien?.tenNhanVien}</p>
                  <span className='text-base text-gray-500'>{employee?.vaiTro}</span>
                  <span className='text-blue-500'>
                    {employee?.nhanVien?.email ?? "email@example.com"}
                  </span>
                </div>
              </div>
            </Popover.Panel>
