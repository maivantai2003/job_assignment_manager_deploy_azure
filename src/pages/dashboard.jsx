...
import {
  fetchStatisticsTask,
  fetchStatisticsEmployee,
  fetchStatisticsDepartment,
...
const barData = [
  { name: "Hoàn thành", value: 4, color: "#4CAF50" },
  { name: "Chưa hoàn thành", value: 0, color: "#F44336" },
  { name: "Trì hoãn", value: 0, color: "#2196F3" },
];

const pieData = [{ name: "Incomplete", value: 4, color: "#FF6384" }];
  ...
  <div style={styles.chartBox}>
    ...
    <div style={styles.donutChartContainer}>
      <div style={styles.donutChart}>
        ...
        <div
          style={{
            ...styles.donutRing,
            background: `conic-gradient(${pieData[0].color} 360deg, ${pieData[0].color} 360deg)`,
          }}
        ></div>
    ...
    <div style={styles.legend}>
      <div style={styles.legendItem}>
        <div
          style={{ ...styles.legendColor, backgroundColor: pieData[0].color }}
        ></div>
  ...
  <div style={styles.chartBox}>
    ...
    <div style={styles.lollipopChart}>
      {lollipopData.map((item, index) => (
        <div
          key={index}
          style={{ ...styles.lollipopItem, height: `${item.value * 20}px` }}
        >
  ...
  <div style={styles.chartBox}>
    ...
    <div style={styles.lineChart}>
      ...
      <div style={styles.lineChartContent}>
        {[0, 1, 2, 3].map((value) => (
          <div
            key={value}
            style={{ ...styles.gridLine, bottom: `${value * 25}%` }}
          />
        ))}
      ...
      <div style={styles.xAxis}>
        {lineData.map(
          (item, index) => index % 2 === 0 && <div key={index}>{item.name}</div>
        )}
      </div>
    </div>
    <div style={styles.lineLegend}>
      <span style={styles.lineLegendItem}>
        <span
          style={{ ...styles.lineLegendColor, backgroundColor: "#E0E0E0" }}
        ></span>
        Total
      </span>
      <span style={styles.lineLegendItem}>
        <span
          style={{ ...styles.lineLegendColor, backgroundColor: "#8884D8" }}
        ></span>
        Completed
      </span>
    </div>
...
function App() {
  ...
  const [statisticsEmployee, setStatisticsEmployee] = useState([]);
  const [statisticsDepartment, setStatisticsDepartment] = useState([]);
  ...
  const filteredTasks = statistics.filter((task) => {
    ...
    const isStartDateValid = filterStartDate
  ...
  const barData = [
    { name: "Hoàn thành", value: completedTasks, color: "#4CAF50" },
    { name: "Chưa hoàn thành", value: incompleteTasks, color: "#F44336" },
    { name: "Trì hoãn", value: overdueTasks, color: "#2196F3" },
  ];
  const exportToPDF = () => {
    const doc = new jsPDF();
    ...
    const taskDetails = statistics.map((task) => {
      return [
        task.maCongViec,
        task.tenCongViec,
        moment(task.thoiGianBatDau).format("DD/MM/YYYY HH:mm"),
        moment(task.thoiGianKetThuc).format("DD/MM/YYYY HH:mm"),
        task.mucDoHoanThanh,
  ...
  const exportToExcel = async (type) => {
    let reportData = [];
    console.log(await dispatch(fetchStatisticsEmployee()).unwrap())
    console.log(await dispatch(fetchStatisticsDepartment()).unwrap())
    try {
      if (type === "summary") {
        const statisticsEmployees = await dispatch(fetchStatisticsEmployee()).unwrap();
        
        reportData = [
          ["Mã Nhân Viên", "Tên Nhân Viên", "Hoàn Thành", "Chưa Hoàn Thành","Tổng Phân Công", "Phần Trăm"],
          ...statisticsEmployees.map((emp) => [
            emp.maNhanVien,
            emp.tenNhanVien,
            emp.phanCongHoanThanh,
            emp.phanCongChuaHoanhThanh,
            emp.phanCongHoanThanh+emp.phanCongChuaHoanhThanh,
            emp.phanTramPhanCong.toFixed(2),
          ]),
        ];
      } else if (type === "detailed") {
        const statisticsDepartments = await dispatch(fetchStatisticsDepartment()).unwrap();
        
        reportData = [
          ["Mã Phòng Ban", "Tên Phòng Ban", "Hoàn Thành", "Chưa Hoàn Thành","Tổng Phân Công", "Phần Trăm"],
          ...statisticsDepartments.map((dep) => [
            dep.maPhongBan,
            dep.tenPhongBan,
            dep.phanCongHoanThanh,
            dep.phanCongChuaHoanhThanh,
            dep.phanCongHoanThanh+dep.phanCongChuaHoanhThanh,
            dep.phanTramPhanCong.toFixed(2),
          ]),
        ];
      }
      ...
      XLSX.utils.book_append_sheet(wb, ws, type === "summary" ? "Employee Report" : "Department Report");
  
      const fileName = type === "summary" ? "Employees.xlsx" : "Departments.xlsx";
      XLSX.writeFile(wb, fileName);
  
    } catch (error) {
      toast.error('Xuất báo cáo không thành công!'+error);
    }
  };
